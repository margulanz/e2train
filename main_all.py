...
def parse_args():
    ...
    parser.add_argument('arch', metavar='ARCH',
                        ...
                        help='model architecture: ' +
                             ' | '.join(model_names) +
                             ' (default: cifar10_rnn_gate_74)')
    parser.add_argument('--SMD', default=False)
    ...
    parser.add_argument('--predictive_backward', default=False, type=str2bool,
...
def main():
    ...
    smb_iterations = [64000,58600,53333, 48000,42666,37333,32000]
    smd_iterations = [128000,117200,106666,96000,83200,74666,64000]
    smb_accuracies = []
    smd_accuracies = []
    if args.cmd == 'train':
        ...
        for iters in [el//2 for el in smd_iterations]:
            logging.info('training using SMD with {} number of iterations'.format(iters))
            args.iters = iters
            accuracy = run_training(args)
            smb_accuracies.append(accuracy)
            logging.info('training using SMD with {} number of iterations finished with accuracy: {}'.format(iters,accuracy))
        
        logging.info(smb_accuracies)
        logging.info(smd_accuracies)
...
def run_training(args):
    ...
    for i in range(0, args.iters):
        # SMD PART IMPLEMENTATION
        rand_flag = random.uniform(0, 1) > 0.5 if args.SMD else 0
        ######################################
        model.train()
        adjust_learning_rate(args, optimizer, i,args.iters)
...
def adjust_learning_rate(args, optimizer, _iter, N_iter):
    """divide lr by 10 at 32k and 48k """
    first = N_iter/2
    second = N_iter*3/4
    if args.warm_up and (_iter < 400):
        lr = 0.01
    elif int(first * 4/3) <= _iter < int(second * 4/3):
        lr = args.lr * (args.step_ratio ** 1)
    elif _iter >= int(second * 4/3):
